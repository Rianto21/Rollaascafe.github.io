                    ...
                    <tr class="border-2 border-green-700 py-4 px-6 rounded-lg flex justify-between items-center">
                        ...
                        <td class="flex gap-8 items-center">
                            <div class="flex gap-3 items-center">
                                <div class="bg-green-700 text-white rounded-md py-2 px-4 text-lg" :class="[cart.jumlah_barang > 1 ? 'cursor-pointer' : 'cursor-default']" @click="cart.jumlah_barang > 1 ? cart.jumlah_barang-- : cart.jumlah_barang = cart.jumlah_barang, updateKeranjang()">-</div>
                                <div class="text-xl text-green-700 font-semibold">{{cart.jumlah_barang}}</div>
                                <div class="bg-green-700 text-white rounded-md py-2 px-4 text-lg cursor-pointer" @click="cart.jumlah_barang++, updateKeranjang()">+</div>
                            </div>
...
export default {
    ...
    methods: {
        updateKeranjang() {
            ...
            axios.patch('https://rollaascafeapinodejs.herokuapp.com/users/cart/'+this.session, this.carts)
            .then(() => console.log("ok"))
